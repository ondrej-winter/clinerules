...
5. Mind shell escaping. If the artifact contains quotes, `$(...)`, or backticks, write the complete prompt to a file and pass that file through stdin instead of interpolating the artifact into an inline argument. When in doubt, ask the user to confirm the invocation before running it.

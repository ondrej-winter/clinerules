...
- Prefer `author-agent-skill/SKILL.md` from the same selected skill root when it exists.
- Otherwise use `shared/agents/skills/author-agent-skill/SKILL.md` as the source-of-truth fallback.
...
def selected_roots(args: argparse.Namespace) -> list[Path]:
    """Return existing skill roots selected by arguments."""

    roots = args.root or list(DEFAULT_SKILL_ROOTS)
    resolved = [resolve_repo_path(root) for root in roots]
    return [root for root in resolved if root.exists()]
...
def discover_skill_dirs(roots: list[Path]) -> list[Path]:
    ...
    return sorted(skill_dirs, key=repo_relative)
    ...
    payload = [
        {
            ...
            'log': repo_relative(result.task.log_path),
        }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_by_task):
                ...
                try:
                    ...
                    print(f'{status} {task.relative_path} -> {repo_relative(task.log_path)}')
